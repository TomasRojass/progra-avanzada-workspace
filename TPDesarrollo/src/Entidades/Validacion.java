...
public class Validacion {
	...
	public static int verificarRangos(int presupuesto, int cantidadDeFabricantes, int cantidadDeCompradores) {
		return (presupuesto >= LIMITE_INFERIOR_PRESUPUESTO && presupuesto <= LIMITE_SUPERIOR_PRESUPUESTO)
				&& (cantidadDeFabricantes >= LIMITE_INFERIOR_PERSONAS
						&& cantidadDeFabricantes <= LIMITE_SUPERIOR_PERSONAS)
				&& (cantidadDeCompradores >= LIMITE_INFERIOR_PERSONAS
						&& cantidadDeCompradores <= LIMITE_SUPERIOR_PERSONAS) ? Constante.TODO_OK
								: Constante.ERROR_RANGOS;
	}

	public static int verificarPrecioYCantidad(int precio, int cantidad) {
		return (precio >= LIMITE_INFERIOR_PRECIO && precio <= LIMITE_SUPERIOR_PRECIO)
				&& (cantidad >= LIMITE_INFERIOR_CANTIDAD && cantidad <= LIMITE_SUPERIOR_CANTIDAD) ? Constante.TODO_OK
						: Constante.ERROR_RANGOS;
	}
