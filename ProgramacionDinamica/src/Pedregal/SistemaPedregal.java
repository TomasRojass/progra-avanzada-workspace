...
public class SistemaPedregal {
	...
	/**
	 * Intenta construir una casa en el terreno utilizando una estrategia de bottom-up.
	 * Se utiliza una matriz de acumulación de piedras para verificar si hay espacio suficiente
	 * para la casa en cada posible ubicación del terreno.
	 * 
	 * @param plan El objeto PlanConstruccion que contiene la información del terreno y la casa.
	 */
	private void construirCasaBottomUp(PlanConstruccion plan) {
		...
		/** 
		 * Todas las sumas de +1 en los indices es porque el archivo viene desde fila y columna 1
		 */
	...
	/**
	 * Crea una matriz de acumulación basada en la matriz del terreno de PlanConstruccion.
	 * La matriz de acumulación facilita el cálculo de la suma de elementos en submatrices del terreno.
	 * 
	 * - La primera fila se acumula sumando valores de izquierda a derecha.
	 * - La primera columna se acumula sumando valores de arriba a abajo.
	 * - Otras celdas se calculan sumando el valor de la celda de arriba, la celda a la izquierda,
	 ...
	 * @return Una matriz de acumulación con las mismas dimensiones que la matriz del terreno.
	 */
	...
	/**
	 * Intenta ubicar una casa en el terreno especificado por el objeto PlanConstruccion.
	 * Verifica si hay espacio suficiente en la matriz de acumulación de piedras para colocar la casa.
	 *
	 * - La verificación se realiza utilizando la matriz de acumulación de piedras para determinar si la
	 *   submatriz correspondiente al espacio de la casa está libre de piedras.
	 * - Si se encuentra un lugar adecuado, se establece la ubicación de la casa en el plan.
	 *
	 * @param plan El objeto PlanConstruccion que contiene la matriz del terreno y las dimensiones de la casa.
	 ...
	 */
	private void intentarFabricarCasa(PlanConstruccion plan, int frente, int lado, int i, int j, int[][] matrizPiedras) {
		...
		if(j - lado + 1 == 0) {
			...
			if(i - frente + 1 > 0) {
				hayLugar = matrizPiedras[i][j] - matrizPiedras[i - frente][j] - matrizPiedras[i][j - lado] + matrizPiedras[i - frente][j - lado] == 0;
			}
		}
		if(hayLugar) {
