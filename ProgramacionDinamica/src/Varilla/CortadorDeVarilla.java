...
public class CortadorDeVarilla {
	...
	/**
	 * Calcula el costo mínimo de cortar una varilla de longitud `largoVarilla` en
	 * las posiciones especificadas en `cortesVarilla`, utilizando un enfoque
	 * top-down de programación dinámica con memoización.
	 *
	 * @param cortesVarilla Un array de enteros que indica las posiciones en las que
	 *                      se deben realizar los cortes.
	 * @param largoVarilla  La longitud total de la varilla.
	 * @return El costo mínimo de realizar todos los cortes.
	 */
	public int cortarOptimoTopDown(int largoVarilla, int[] cortesVarilla) {
	...
	/**
	 * Metodo auxiliar recursiva que calcula el costo mínimo de realizar los cortes
	 * en la varilla en el rango especificado por `izq` y `der`, utilizando
	 * memoización para evitar cálculos repetidos.
	 *
	 * @param cortesVarilla Un array de enteros que indica las posiciones de los
	 *                      cortes, incluyendo los extremos.
	 * @param izq           El índice izquierdo del rango actual en el que se están
	 *                      considerando los cortes.
	 * @param der           El índice derecho del rango actual en el que se están
	 *                      considerando los cortes.
	 * @param memo          Un HashMap que almacena los costos mínimos calculados
	 *                      para las combinaciones de cortes.
	...
	private int cortarOptimoTopDown(int[] cortesVarilla, int izq, int der, HashMap<CombinacionCorte, Integer> memo) {
		...
		for (int i = izq + 1; i < der; i++) {
			this.iteraciones++;
			// Calcula el costo de cortar en la posición actual y los costos de los cortes
			// recursivos
			int costo = cortesVarilla[der] - cortesVarilla[izq] + cortarOptimoTopDown(cortesVarilla, izq, i, memo)
					+ cortarOptimoTopDown(cortesVarilla, i, der, memo);
			if (costo < minCosto) {
				minCosto = costo;
			}
		}
