...
public class SumaDeSubrectangulos {
	...
	/**
	 * Calcula la suma máxima de cualquier subrectángulo en una matriz dada. Utiliza
	 * una matriz intermedia de sumas acumuladas para facilitar el cálculo de sumas
	 * de submatrices.
	 *
					...
					while (ubicCol < matriz[0].length && ubicCol + tamCol <= matrizIntermedia[0].length) {
						...
						if (ubicFila - tamFila == -1) { // si esta en primera fila
							if (ubicCol - tamCol == -1) { // si esta en primera fila primera columna
								...
								sumaSubMatriz = matrizIntermedia[ubicFila][ubicCol]
										- matrizIntermedia[ubicFila][ubicCol - tamCol];
							}
						} else if (ubicCol - tamCol == -1) { // si esta en primera columna
							sumaSubMatriz = matrizIntermedia[ubicFila][ubicCol]
									- matrizIntermedia[ubicFila - tamFila][ubicCol];
						} else {
							sumaSubMatriz = matrizIntermedia[ubicFila][ubicCol]
									- matrizIntermedia[ubicFila][ubicCol - tamCol]
									- matrizIntermedia[ubicFila - tamFila][ubicCol]
									+ matrizIntermedia[ubicFila - tamFila][ubicCol - tamCol];
						}
	...
	/**
	 * Genera una matriz intermedia de sumas acumuladas para facilitar el cálculo de
	 * sumas de submatrices. La matriz intermedia en cada posición (i, j) contiene
	 * la suma de todos los elementos en el subrectángulo que va desde (0, 0) hasta
	 * (i, j) en la matriz original.
	 *
	 * @param matriz La matriz de enteros de entrada.
		...
		for (int i = 0; i < cantFilas; i++) {
			for (int j = 0; j < cantCol; j++) {
				if (i == 0) {
					...
					matrizSuma[i][j] = matriz[i][j] + matrizSuma[i][j - 1] + matrizSuma[i - 1][j]
							- matrizSuma[i - 1][j - 1];
				}
			}
		}
